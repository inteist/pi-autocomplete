...
/**
 * Diagnostic function executed by the `/autocomplete-check` command.
 * It:
 ...
 * 
 * It:
 * 1. Sanitizes line endings and removes model special tokens (e.g., FIM, chat, templates).
 * 2. Strips prompt echoes that the model may have repeated.
 * 3. Standardizes code structures (removes enclosing quotes, prefix continuation labels).
 * 4. Extracts only the first logical paragraph to prevent multi-paragraph block spam.
 * 5. Caps length at 200 characters to keep autocomplete completions concise.
 *
...
/**
 * Parses user input arguments for the `/autocomplete-model` command.
...
/**
 * Maps short model names/aliases to their full Ollama registry name equivalents
 * (e.g., "qwen" to "qwen2.5-coder:1.5b", "gemma" to "gemma4:e2b").
 */
...
/**
 * Returns a human-readable description of the configured prompt mode,
 * indicating if it was automatically resolved.
 */
